# 0 "fir_control.c"
# 1 "/home/ubuntu/soc/lab-caravel_fir/testbench/counter_la_fir//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "fir_control.c"
# 19 "fir_control.c"
...
# 70 "fir_control.c"
 ...
 for (int n=0; n<3; n++) {
  ...
  for (int i=0; i<11; i++) {
   ...
   (&(*(volatile uint32_t*)0x30000040))[i] = taps[i];
  }
  ...
  int y[64];
  for (int i=0; i<64; i++) {
   while (((*(volatile uint32_t*)0x30000000) & 0x10) != 0x10);
   (*(volatile uint32_t*)0x30000080) = i;

   while (((*(volatile uint32_t*)0x30000000) & 0x20) != 0x20);
   y[i] = (*(volatile uint32_t*)0x30000084);

   if (i == 64 -1) {

    (*(volatile uint32_t*)0x2600000c) = (y[i] << 24) + 0x005A0000;
   }
   else {
    (*(volatile uint32_t*)0x2600000c) = (y[i] << 24);
   }
  }
  while (((*(volatile uint32_t*)0x30000000) & 0x02) != 0x02);
 }
